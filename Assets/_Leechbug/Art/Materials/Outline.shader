Shader "Hidden/Outline"
{
    ...
    float4 FullScreenPass(Varyings varyings) : SV_Target
    {
        ...
        if (Luminance(outline.rgb) < luminanceThreshold)
        {
            ...
            {
                //min needs to run 1 time to get just a one pixel outline
                for (int j = 1; j < OUTLINEWIDTH + 1; j++) {
                    float2 uvN = uv + _ScreenSize.zw * _RTHandleScale.xy *j* samplingPositions[i];
